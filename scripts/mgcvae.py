...
if __name__ == "__main__":
    ...
    parser.add_argument('--conditions', type=str, default='../data/conditions.csv',
                        help="conditions path (default='../data/conditions.csv')")
    ...
    print()
    print('Small Molecular Graph Conditional Variational Autoencoder for Multi-objective Optimization')
    print('(logP & Molar Refractivity)')
    print()
    print('- Laboratory:')
    ...
    for i in range(len(data_list)):
        train_list.append([np.array([np.array(data_list[i])]),
                           np.array(logp_list[i]), np.array(mr_list[i])])
    ...
    z_dim = 128
    cvae = CVAE(x_dim=out_dim, h_dim1=512, h_dim2=256,
                z_dim=z_dim, c_dim=cond_dim)
